...
if fps == 0 or np.isnan(fps):
    fps = 30.0
print(f"Original Video Resolution: {orig_w}x{orig_h} @ {fps} FPS")
...
def video_capture_thread(cap_obj, target_fps):
    global latest_frame
...
cap_thread = threading.Thread(target=video_capture_thread, args=(cap, fps), daemon=True)
cap_thread.start()
